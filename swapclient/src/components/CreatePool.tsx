...
const CreatePool = ({ getAllTokens }: CreatePoolProps) => {
  ...
  useEffect(() => {
    if (open && dialogContentRef.current) {
      ...
      gsap.set([headerRef.current, tokenGridRef.current, footerRef.current], {
        opacity: 0,
        y: 20,
      });
  ...
  useEffect(() => {
    ...
    if (previewRef.current && isTokenAValid && isTokenBValid) {
      gsap.fromTo(
        previewRef.current,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent
        className="w-[90vw] sm:w-[85vw] md:w-[75vw] lg:w-[70vw] xl:w-[60vw] max-w-4xl mx-auto p-0 rounded-lg sm:rounded-xl border-0 shadow-2xl overflow-hidden bg-card text-card-foreground"
        ref={dialogContentRef}
      >
        <div className="px-4 sm:px-6 py-3 sm:py-4" ref={headerRef}>
          <DialogHeader className="space-y-1 sm:space-y-2">
            <DialogTitle className="text-lg sm:text-xl font-bold flex items-center gap-2 text-primary">
              <PlusSquare className="w-5 h-5 sm:w-6 sm:h-6 text-primary" />
              <span className="hidden xs:inline">Create New Pool</span>
              <span className="inline xs:hidden">Create Pool</span>
            </DialogTitle>
        ...
        <div className="px-4 sm:px-6 pb-4 sm:pb-6 space-y-4 sm:space-y-6">
              ...
              <div className="space-y-2 sm:space-y-3">
                <div className="space-y-1 sm:space-y-2">
                  ...
                  <Input
                    ...
                    className={`h-10 sm:h-12 text-sm sm:text-base border-2 transition-all duration-200 ${
                      tokenA.tokenAddress &&
                  ...
                  <div className="space-y-1 sm:space-y-2 flex-1">
                    <div className="relative">
                      <Input
                        ...
                        value={tokenA.name}
                        readOnly
                        className="h-10 sm:h-12 text-sm sm:text-base bg-muted border-border"
                      />
                      {tokenALoading && (
                  ...
                  <div className="space-y-1 sm:space-y-2 flex-1">
                    ...
                    <div className="relative">
                      ...
                      {!tokenALoading && tokenA.symbol && (
                        <div className="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2">
                          <span className="text-[10px] sm:text-xs font-medium text-muted-foreground bg-muted px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">
                            {tokenA.symbol}
                          </span>
                        </div>
                      )}
          ...
          {isTokenAValid && isTokenBValid && (
            <div
              className="bg-green-50 border border-green-400 rounded-lg p-3 sm:p-4"
              ref={previewRef}
            >
          ...
          (tokenB.tokenAddress && !isValidAddress(tokenB.tokenAddress)) ? (
            <div className="bg-destructive/10 border border-destructive rounded-lg p-3 sm:p-4">
              <div className="flex items-start gap-2 sm:gap-3">
                <svg
                  className="w-4 h-4 sm:w-5 sm:h-5 text-red-500 mt-0.5 shrink-0"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
          ...
          {creating && creatingStep && (
            <div className="bg-accent/10 border border-accent rounded-lg p-3 sm:p-4">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-4 h-4 sm:w-5 sm:h-5 border-2 border-amber-600 border-t-transparent rounded-full animate-spin shrink-0"></div>
                <div>
        ...
        <DialogFooter
          className="px-4 sm:px-6 py-3 sm:py-4 border-t border-border bg-card"
          ref={footerRef}
        >
          <Button
            onClick={createPool}
            disabled={!canCreate}
            className="w-full h-10 sm:h-12 font-semibold text-sm sm:text-base rounded-lg shadow-sm hover:shadow-md transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 bg-primary text-primary-foreground hover:bg-primary/90"
          >
            {creating ? (
              <div className="flex items-center gap-1.5 sm:gap-2">
                <div className="w-3 h-3 sm:w-4 sm:h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                <span className="text-xs sm:text-base">
                  {creatingStep || "Creating Pool..."}
                </span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 sm:gap-2">
                <PlusSquare className="w-4 h-4 sm:w-5 sm:h-5" />
                <span className="text-xs sm:text-base">Create Pool</span>
              </div>
            )}
          </Button>
        </DialogFooter>
